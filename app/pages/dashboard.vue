  ...
  <div class="min-h-screen bg-default">
    <div class="border-b border-default bg-elevated/80 backdrop-blur">
      <UContainer class="flex flex-col gap-4 py-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <UButton color="neutral" variant="subtle" icon="i-lucide-log-out" label="Logout" @click="logout" />
        </div>
      </UContainer>
    </div>
    ...
    <UContainer class="grid gap-6 py-6 lg:grid-cols-[240px_minmax(0,1fr)]">
      ...
      <main class="space-y-6">
        ...
        <div v-else-if="wigs.length === 0" class="rounded-2xl border border-dashed border-default bg-muted/20 p-10 text-center">
        ...
        <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <NuxtLink
            ...
            class="block"
          >
            <UCard class="h-full transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
              <template #header>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-2">
                  <div class="flex items-center justify-between gap-4 text-sm">
                    ...
                    <span class="text-muted">{{ wigProgress(record.currentValue, record.targetValue) }}%</span>
                  </div>
                  <UProgress :model-value="wigProgress(record.currentValue, record.targetValue)" color="primary" />
                </div>

                <div class="flex items-center justify-between gap-4 text-sm text-muted">
                  <span>Deadline</span>
                  <span>{{ new Date(record.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }}</span>
                </div>
              </div>
            </UCard>
          </NuxtLink>
